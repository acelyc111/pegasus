...
int pegasus_write_service::batch_put(const db_write_context &ctx,
                                     ...
                                     dsn::apps::update_response &resp)
{
    CHECK(_batch_start_time != 0, "batch_put must be called after batch_prepare");
...
int pegasus_write_service::batch_remove(int64_t decree,
                                        ...
                                        dsn::apps::update_response &resp)
{
    CHECK(_batch_start_time != 0, "batch_remove must be called after batch_prepare");
...
{
    CHECK(_batch_start_time != 0, "batch_commit must be called after batch_prepare");
...
{
    CHECK(_batch_start_time != 0, "batch_abort must be called after batch_prepare");

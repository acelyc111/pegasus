...
void pegasus_server_impl::on_get_scanner(get_scanner_rpc rpc)
{
    ...
    if (request.hash_key_filter_type == ::dsn::apps::filter_type::FT_MATCH_PREFIX &&
        ...
        if (prefix_start.compare(start) > 0) {
            ...
            PGSCHECK(!_data_cf_opts.prefix_extractor || rd_opts.total_order_seek, "Invalid option");
            PGSCHECK(!_data_cf_opts.prefix_extractor || !rd_opts.prefix_same_as_start,
                     "Invalid option");
        }
    }
...
{
    if (!_is_open) {
        CHECK_EQ(_db, nullptr);
        PGSCHECK(!clear_state, "should not be here if do clear");
        return ::dsn::ERR_OK;
    }
                                          ...
                                          const ::dsn::blob &value)
{
    ...
    default:
        PGSCHECK(false, "unsupported filter type: {}", filter_type);
    }
    return false;
}

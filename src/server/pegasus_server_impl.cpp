...
::dsn::error_code pegasus_server_impl::start(int argc, char **argv)
{
    ...
    if (use_exist_db && check_meta_cf(path, &need_create_meta_cf) != ::dsn::ERR_OK) {
...
::dsn::error_code pegasus_server_impl::check_meta_cf(const std::string& path, bool* need_create_meta_cf) {
    *need_create_meta_cf = true;
    ...
    s = rocksdb::DB::ListColumnFamilies(rocksdb::DBOptions(), path, &column_families);
    if (!s.ok()) {
    ...
    for (const auto& column_family : column_families) {
        if (column_family.GetName() == META_COLUMN_FAMILY_NAME) {

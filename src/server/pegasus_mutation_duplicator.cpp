    ...
    : mutation_duplicator(r), _remote_cluster(remote_cluster)
{
    ...
    CHECK_PREFIX(ret.is_ok(), // never possible, meta server disallows such remote_cluster.
    ...
    CHECK_PREFIX(get_current_cluster_id() != _remote_cluster_id,
                                                     ...
                                                     dsn::error_code err)
{
    ...
    if (perr != PERR_OK || err != dsn::ERR_OK) {
        ...
        CHECK_PREFIX(perr != PERR_INVALID_ARGUMENT, rpc.response().error_hint);
    } else {

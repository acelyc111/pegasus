...
public:
    ...
    gpid before_check_partition_status(bulk_load_status::type status)
    {
        ...
        SET_IPS_AND_HOST_PORTS_BY_DNS(pc, secondaries1, SECONDARY1_HP, SECONDARY2_HP);
        app->pcs.clear();
                                ...
                                bool always_unhealthy_check)
    {
        ...
        if (mock_primary_invalid) {
            RESET_IP_AND_HOST_PORT(app->pcs[pid.get_partition_index()], primary1);
        }
        if (mock_lack_secondary) {
            CLEAR_IP_AND_HOST_PORT(app->pcs[pid.get_partition_index()], secondaries1);
        }
        partition_configuration pc;
                                      ...
                                      bool use_secondary3 = false)
    {
        ...
        pinfo.host_ports.emplace_back(PRIMARY_HP);
        pinfo.host_ports.emplace_back(SECONDARY1_HP);
        if (use_secondary3) {
            pinfo.host_ports.emplace_back(SECONDARY3_HP);
        } else {
            pinfo.host_ports.emplace_back(SECONDARY2_HP);
        }
                                   ...
                                   bool same)
    {
        ...
        SET_IP_AND_HOST_PORT_BY_DNS(pc, primary1, PRIMARY_HP);
        if (same) {
            ADD_IP_AND_HOST_PORT_BY_DNS(pc, secondaries1, SECONDARY1_HP);
            ADD_IP_AND_HOST_PORT_BY_DNS(pc, secondaries1, SECONDARY2_HP);
        } else {
            ADD_IP_AND_HOST_PORT_BY_DNS(pc, secondaries1, SECONDARY1_HP);
            if (secondary_count == 2) {
                ADD_IP_AND_HOST_PORT_BY_DNS(pc, secondaries1, SECONDARY3_HP);

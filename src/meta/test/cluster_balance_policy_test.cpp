...
{
    {
        ...
        info1.address = host_port(1, 10086);
        cluster_balance_policy::node_migration_info info2;
        info2.address = host_port(2, 10086);
        ASSERT_LT(info1, info2);
    }

    {
        cluster_balance_policy::node_migration_info info1;
        info1.address = host_port(1, 10000);
        cluster_balance_policy::node_migration_info info2;
        info2.address = host_port(1, 10086);
        ASSERT_LT(info1, info2);
    }
...
TEST(cluster_balance_policy, get_app_migration_info)
{
    ...
    auto address = host_port(1, 10086);
    app_info info;
    info.app_id = appid;
    info.app_name = appname;
    info.partition_count = 1;
    auto app = std::make_shared<app_state>(info);
    app->partitions[0].host_port_primary = address;
...
TEST(cluster_balance_policy, get_min_max_set)
{
    ...
    ASSERT_EQ(*min_set.begin(), host_port(1, 10086));
    ASSERT_EQ(max_set.size(), 2);
    ASSERT_EQ(*max_set.begin(), host_port(3, 10086));
    ASSERT_EQ(*max_set.rbegin(), host_port(4, 10086));
}
...
TEST(cluster_balance_policy, pick_up_partition)
{
...

    {
        ...
        partition_set selected_pid;
        host_port not_exist_addr(3, 12345);
...
{
    auto addr1 = host_port(1, 1);
    auto addr2 = host_port(1, 2);
    auto addr3 = host_port(1, 3);

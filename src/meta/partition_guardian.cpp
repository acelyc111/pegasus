...
void partition_guardian::reconfig(meta_view view, const configuration_update_request &request)
{
    ...
    if (request.type == config_type::CT_DROP_PARTITION) {
        ...
        // TODO(yingchun): both
        const std::vector<host_port> &config_dropped = request.config.host_port_last_drops;
        for (const host_port &drop_node : config_dropped) {
                                        ...
                                        configuration_proposal_action &action)
{
    ...
    if (!is_node_alive(*(view.nodes), action.host_port_node)) {
        sprintf(reason, "action node(%s) is not alive", action.host_port_node.to_string().c_str());
        goto invalid_action;
    }
...
pc_status partition_guardian::on_missing_primary(meta_view &view, const dsn::gpid &gpid)
{
    ...
    // just find a node and assign primary for it.
        ...
        if (min_primary_server_np != nullptr) {
            action.host_port_node = min_primary_server;
            action.host_port_target = action.host_port_node;
    ...
    if (is_emergency) {
        ...
        while (cc.prefered_dropped >= 0) {
            ...
            if (is_node_alive(*view.nodes, server.node)) {
                ...
                action.host_port_node = server.node;
                cc.prefered_dropped--;
                break;
            } else {
...
pc_status partition_guardian::on_redundant_secondary(meta_view &view, const dsn::gpid &gpid)
{
    ...
    action.host_port_node = pc.host_port_secondaries[target];
    action.host_port_target = pc.host_port_primary;

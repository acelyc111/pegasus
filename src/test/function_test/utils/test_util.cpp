                             ...
                             int count) const
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        case OperateDataType::kSet:
            ...
            ASSERT_EQ(PERR_OK, client->set(hashkey, kDefaultSortkey, value));
            break;
        case OperateDataType::kGet: {
            std::string value_new;
            ASSERT_EQ(PERR_OK, client->get(hashkey, kDefaultSortkey, value_new));
            ASSERT_FALSE(value.empty());
            ASSERT_EQ(value, value_new);
            break;
        }
        case OperateDataType::kDelete:
            ASSERT_EQ(PERR_OK, client->del(hashkey, kDefaultSortkey));
            break;
        case OperateDataType::kCheckNotFound: {
            std::string value_new;
            ASSERT_EQ(PERR_NOT_FOUND, client->get(hashkey, kDefaultSortkey, value_new));
...
{
    NO_FATALS(write_data(kDefaultHashkeyPrefix, kDefaultValuePrefix, count));
}
...
{
    NO_FATALS(verify_data(table_name, kDefaultHashkeyPrefix, kDefaultValuePrefix, count));
}

void test_util::verify_data(int count) const
{
    NO_FATALS(verify_data(table_name_, kDefaultHashkeyPrefix, kDefaultValuePrefix, count));
}

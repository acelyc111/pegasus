...
{
    CHECK(hash_key.length() < UINT16_MAX, "hash key length must be less than UINT16_MAX");
...
{
    CHECK(key.length() >= 2, "key length must be no less than 2");

    // hash_key_len is in big endian
    uint16_t hash_key_len = ::dsn::endian::ntoh(*(uint16_t *)(key.data()));

    if (hash_key_len > 0) {
        CHECK(key.length() >= 2 + hash_key_len,
...
{
    CHECK(key.size() >= 2, "key length must be no less than 2");
    ...
    if (hash_key_len > 0) {
        ...
        CHECK(key.size() >= 2 + hash_key_len, "key length must be no less than (2 + hash_key_len)");

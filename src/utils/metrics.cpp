...
{
    CHECK_F(attrs.find("entity") == attrs.end(), "{}'s attribute \"entity\" is reserved", id);
...
#define TRY_PROCESS_TIMER_CLOSING()                                                                \
    ...
    if (dsn_unlikely(!!ec)) {
        CHECK_F(ec == boost::system::errc::operation_canceled,
                "failed to exec on_timer with an error that cannot be handled: {}",
                ec.message());

        // Cancel can only be launched by close().
        auto expected_state = state::kClosing;
        CHECK_F(_state.compare_exchange_strong(expected_state, state::kClosed),

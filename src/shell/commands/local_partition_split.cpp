...
{
    static const std::string kRdbDirPostfix = dsn::utils::filesystem::path_combine(
        dsn::replication::replication_app_base::kDataDirPostfix,
        dsn::replication::replication_app_base::kRdbPostfix);
...
{
    static const std::string kReplicasDirPostfix =
        dsn::utils::filesystem::path_combine(dsn::replication::replication_options::kAppType,
    ...
    const auto dst_replicas_dir =
        dsn::utils::filesystem::path_combine(dst_data_dir, kReplicasDirPostfix);
...
bool local_partition_split(command_executor *e, shell_context *sc, arguments args)
{
    ...
    std::ostream out(buf);
    tp.output(out, tp_output_format::kTabular);

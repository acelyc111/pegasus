...
class ServerOperator(object):
    ...
    def modify_conf(cls, old_conf, new_conf):
        ...
        status, output = getstatusoutput(
            'sed -i "s/%s/%s/" %s' % (old_conf, new_conf, origin_conf_file)
        )
        # print(status, output)
    ...
    def start_cluster(cls, meta_count, replica_count, check_health):
        status, output = getstatusoutput(
            "cd %s && ./run.sh start_onebox -m %s -r %s"
    ...
    def stop_and_clear_cluster(cls):
        status, output = getstatusoutput(
            "cd %s && ./run.sh stop_onebox" % cls.shell_path
        )

        status, output = getstatusoutput(
            "cd %s && ./run.sh clear_onebox" % cls.shell_path
        )

    @classmethod
    def operate_1_server(cls, op_type, server_type, index):
        status, output = getstatusoutput(
            "cd %s && ./run.sh %s_onebox_instance -%s %s"
    ...
    def wait_until_cluster_health(cls):
        cmd = (
            ...
            " grep fully_healthy_partition_count | awk '{print $NF}'" % cls.shell_path
        )
        while True:
...
class TestIntegration(unittest.TestCase):
    ...
    def loop_op(self):
        for i in range(self.DATA_COUNT):
            ...
            if (
                ...
                or ret[0] != error_types.ERR_OK.value
                or bytes.decode(ret[1]) != self.TEST_VALUE
            ):
    ...
    def test_can_not_connect(self):
        self.c = Pegasus(
            ["127.0.1.1:34601", "127.0.0.1:34602", "127.0.0.1:34603"], "temp"
        )
        ret = yield self.c.init()
    ...
    def test_1of3_replica_restart(self):
        ...
        for i in range(self.DATA_COUNT):
            (ret, ign) = yield self.c.set(
                self.TEST_HKEY + str(i), self.TEST_SKEY, self.TEST_VALUE
            )
            self.assertEqual(ret, error_types.ERR_OK.value)
